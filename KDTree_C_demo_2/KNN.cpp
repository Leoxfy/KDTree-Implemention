...
void KNN::train(){
    kdTree = build_kdTree(0, training_data);
}
...
void KNN::test(){
    ...
    get_nearest_neighbor(kdTree, test_data, 0, father);
...
void KNN::get_nearest_neighbor(Node*& root, Point& target_point, int layer, Node*& father){
    ...
    // 访问兄弟节点
    double mid_distance;
    mid_distance = fabs(root->root_point.data[axis] - target_point.data[axis]);
